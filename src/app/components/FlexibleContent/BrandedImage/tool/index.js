...
const Tool = ({ brand, bgColour, colour, tool }) => {
    ...
    const sticker0w = stickers[0]?.sticker.mediaDetails.width;
    const sticker0h = stickers[0]?.sticker.mediaDetails.height;
    const sticker0w2 = stickers[0]?.sticker.mediaDetails.sizes[1].width;
    const sticker0h2 = stickers[0]?.sticker.mediaDetails.sizes[1].height;
    
    const firstSticker = stickers ? stickers[0].sticker.sourceUrl : null;
    const firstStickerW = stickers && sticker0w > 0 ? sticker0w : sticker0w2;
    const firstStickerH = stickers && sticker0h > 0 ? sticker0h : sticker0h2;
    ...
    const NotesList = () => {
        ...
        <>
            {stickers && stickers.map((sticker, i) => {
                const w = sticker.sticker.mediaDetails.width > 0 ? sticker.sticker.mediaDetails.width : sticker.sticker.mediaDetails.sizes[1].width;
                const h = sticker.sticker.mediaDetails.height > 0 ? sticker.sticker.mediaDetails.height : sticker.sticker.mediaDetails.sizes[1].height;
                return (
                    <div key={i.toString()} className={`note ${styles.mbMd0} ${loaded && stickerSelected === i ? "active" : !loaded ? "inactive" : ""}`} onClick={() => onNoteChange(sticker.sticker.sourceUrl, w, h, i)} style={{ backgroundColor: colour }}>
        ...
        <div className="position-relative d-flex flex-column align-items-end h-100">
            ...
            >
                {({
                    ...
                    <div className={`${styles.toolWrapper} d-flex flex-column justify-content-${!imageList.length ? 'center' : 'between'} ${imageList.length ? styles.toolWrapperLoaded : ''}`} style={{ borderColor: colour }}>
                                    ...
                                    <div className={styles.imageItem}>
                                        <div className={`${styles.imageItemWrapper} position-relative`} ref={exportRef}>
                                            ...
                                            {loaded && toolSticker && stickerWidth && stickerHeight && <>
                                                <div className="target__wrapper target" style={{ width: firstStickerW + 'px', height: firstStickerH + 'px' }}>
                                    ...
                                    <div className={`${styles.filesWrap} d-flex align-items-center gap-2 align-self-center ${!loaded ? `inactive` : ``}`} onClick={downloadScreenshot}>
                                        <h5 className={`${styles.assetsBtn} d-flex gap-2 align-items-center`}>
                                            <div className='d-flex gap-2 align-items-center'>
                                                <span>Download</span>
                                                <svg width="19" height="21" viewBox="0 0 19 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M9.67501 14.7716L9.67501 1.56396M9.67501 14.7716L5.27246 10.3691M9.67501 14.7716L14.0776 10.3691" stroke={colour} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                                    <line x1="1.51001" y1="19.9648" x2="17.8403" y2="19.9648" stroke={colour} strokeWidth="1.5" strokeLinecap="round"/>
                                                </svg>
                                            </div>
                                        </h5>
                                    </div>}
